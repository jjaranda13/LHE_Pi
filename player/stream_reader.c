...
#define BITS_BUFFER_LENGHT 2000
...
int get_header(int line_num, int *state, int *line_num_real, int *subframe);
...
int decode_stream_stdin(int width, int height) {
	...
	while (true) {
		...
		bits_lenght = buffer_til_nal(stdin, bits, BITS_BUFFER_LENGHT);
		while (index < bits_lenght) {
			int headers, readed_hops, readed_bytes, line_num,
...
int get_header(int line_num, int *state, int *line_num_real, int *subframe) {
	...
	if (line_num >= 256 && line_num < 2256) {
		*state = V_STATE;
		*line_num_real = line_num - 256;
		*subframe = *line_num_real % 24;
		return 0;
	}
	else if (line_num >= 2256 && line_num < 4256) {
		*state = U_STATE;
		*line_num_real = line_num - 2256;
		*subframe = *line_num_real % 24;
		return 0;
	}
	else if (line_num >= 4256) {
		*state = Y_STATE;
		*line_num_real = line_num - 4256;
		*subframe = *line_num_real % 24;
		return 0;
	}
	else {
