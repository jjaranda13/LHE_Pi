...
int decode_stream_3(int width, int height, get_bits_context * ctx) {
	...
	int status, subframe = 0, component_state = Y_STATE, frame_counter = 0, i=0;
	unsigned int subframe_counter[8] = { 0 };
	bool is_Y[1080] = { false }, is_U[1080] = { false },
		is_V[1080] = { false }, first = true;
	uint8_t *hops = NULL;
	...
	while (true) {
		...
		int readed_hops, line_num, past_subframe, readed_width;
		double secs;
		struct timeval t_ini, t_fin;
		uint16_t headers;
		uint8_t header_high;
		...
		switch (component_state) {
		case Y_STATE:
			readed_hops = obtain_symbols_entropic2(ctx, hops, width);
			readed_width = decode_line_quantizer2(hops, img->Y_data + width * line_num, readed_hops);
            if (readed_width != width) {
				printf("Symbols obtained are not the ones expected readed_width=%d width=%d\n", readed_width, width);
			}
			is_Y[line_num] = true;
			subframe_counter[subframe]++;
			break;
		case U_STATE:
			readed_hops = obtain_symbols_entropic2(ctx, hops, width / 2);
			readed_width = decode_line_quantizer2(hops, img->U_data + (width / 2)*line_num, readed_hops);
            if (readed_width != (width/2)) {
				printf("Symbols obtained are not the ones expected readed_width=%d width/2=%d\n", readed_width, (width/2));
			}
			is_U[line_num] = true;
			subframe_counter[subframe]++;
			break;
		case V_STATE:
			readed_hops = obtain_symbols_entropic2(ctx, hops, width / 2);
			readed_width = decode_line_quantizer2(hops, img->V_data + (width / 2)*line_num, readed_hops);
            if (readed_width != (width/2)) {
				printf("Symbols obtained are not the ones expected readed_width=%d width/2=%d\n", readed_width, (width/2));
			}
			is_V[line_num] = true;
...
int decode_stream_stdin(int width, int height) {
	...
	return decode_stream_3(width, height, &ctx);
}
