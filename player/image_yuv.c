...
void upsample_frame_edge(yuv_image *img, yuv_image *img_inter, yuv_image *img_up)
{
    ...
    {
    	scale_edge_sps(img_up->Y_data, img->Y_data, img_up->height, img_up->width);
	}
    else
    {
        for (int y = 0; y< img->height; y++)
    ...
    {
        scale_edge_sps(img_up->U_data, img->U_data, img_up->color_height, img_up->color_width);
        scale_edge_sps(img_up->V_data, img->V_data, img_up->color_height, img_up->color_width);
    }
    else
    {
        for (int y = 0; y< img->height/2; y++)
